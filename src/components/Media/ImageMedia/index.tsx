...
// Extended props to include post context for tracking
...
export const ImageMedia: React.FC<ImageMediaProps> = (props) => {
  ...
  if (!src && resource && typeof resource === 'object') {
    const { alt: altFromResource, height: fullHeight, url, width: fullWidth, id } = resource
  ...
  useEffect(() => {
    if (imageLoaded && !trackedRef.current && (postId || imageId)) {
      ...
      trackImageView({
        ...
        postTitle,
        imageId,
        isRestricted: shouldThrottle,
        userId: currentUser?.id,
        userEmail: currentUser?.email,
      })
    }
  }, [imageLoaded, postId, postTitle, imageId, shouldThrottle, currentUser])
  ...
  // For throttled images, Vercel's free plan automatically throttles images
  ...
  return (
    ...
    >
      <NextImage
        ...
        priority={priority}
        quality={shouldThrottle ? 10 : 100} // Low quality for throttled images
        loading={loading}
        sizes={sizes}
        src={src}
