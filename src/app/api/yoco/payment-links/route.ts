...
export async function POST(request: NextRequest) {
  try {
    ...
    let user: any =
      authResult.user && (authResult.user as any).collection === 'users' ? authResult.user : null
    ...
    if (!user && authTokens.length > 0) {
      for (const token of authTokens) {
        ...
        const tokenAuthResult = await payload.auth({ headers })
        if (tokenAuthResult.user && (tokenAuthResult.user as any).collection === 'users') {
